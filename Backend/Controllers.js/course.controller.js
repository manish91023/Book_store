...
export const createCourse = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        return res.status(500).json({ error: "Error occurred during course creation" });
    }
};
...
export const buyCourses=async(req,res)=>{
    ...
    try {
        ...
        const existingPurchase=await Purchase.find({courseId,userId});
        if(existingPurchase){
            return res.status(400).send({error:"user has already purchased this course"})
        }
        const newPurchase=new Purchase({ userId,courseId});
        await newPurchase.save();
        res.status(200).send({message:"Course  purchased sucessfully ",newPurchase})
